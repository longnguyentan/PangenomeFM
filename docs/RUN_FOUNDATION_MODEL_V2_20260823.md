...
```bash
export PANGENOMEFM_DATA_ROOT=/path/to/high_capacity/pangenomefm_data
export PANGENOMEFM_RESULTS_ROOT=/path/to/pangenomefm_results
...
```bash
export PFM_BENCHMARK_MANIFEST="$PANGENOMEFM_DATA_ROOT/benchmarks/hprc_r2_canonical/manifest.csv"
...
```bash
export PFM_PATH_METADATA="$PANGENOMEFM_DATA_ROOT/path_indexes/hprc_r2/path_metadata.tsv.gz"
...
```bash
export PFM_PATH_EDGE_TABLE=/path/to/path_positive_edges.csv.gz
export PFM_PATH_GFA="$PANGENOMEFM_DATA_ROOT/raw/hprc_r2/pangenome.gfa.gz"
export PFM_PATH_GRAPH_AUDIT=/path/to/gfa_segment_index_audit.json
