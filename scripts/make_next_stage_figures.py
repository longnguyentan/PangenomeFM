...
def figure2() -> None:
    ...
    ax.set_ylim(0, 1)
    ax.axis("off")
    ax.text(0.02, 0.97, "PangenomeFM learns graph structure for reuse across biological analyses", fontsize=18.5, weight="bold", va="top")
    ax.text(
        0.02,
        0.925,
        "To prevent an easy shortcut, the connection being tested is hidden from the model before prediction.",
        fontsize=10.5,
        color=COLORS["muted"],
        va="top",
    )
    ...
    ax.text(0.295, 0.84, "Hide tested connection", fontsize=12.5, weight="bold")
    ...
    ax.text(0.36, 0.59, "connection hidden while the model", ha="center", va="center", fontsize=7.8)
    ax.text(0.36, 0.57, "studies the surrounding graph", ha="center", fontsize=7.8, color=COLORS["rose"], weight="bold")

    # Panel C
    ax.text(0.50, 0.84, "C", fontsize=15, weight="bold", color=COLORS["purple"])
    ax.text(0.535, 0.84, "Two sources of graph context", fontsize=12.5, weight="bold")
    box(ax, 0.50, 0.675, 0.20, 0.105, "Genomic location", "where each segment occurs", face=COLORS["blue_light"], edge=COLORS["blue"], fontsize=8.6)
    box(ax, 0.50, 0.535, 0.20, 0.105, "Graph neighborhood", "nearby branching patterns", face=COLORS["green_light"], edge=COLORS["green"], fontsize=8.6)
    box(ax, 0.735, 0.605, 0.095, 0.105, "Combined profile", "learned graph\nsummary", face=COLORS["purple_light"], edge=COLORS["purple"], fontsize=8.3)
    ...
    ax.text(0.895, 0.84, "Predict connection", fontsize=12.5, weight="bold")
    box(ax, 0.855, 0.58, 0.125, 0.18, "Segment pair", "two profiles\n→ probability", face=COLORS["gold_light"], edge=COLORS["gold"], fontsize=8.8)
    ...
    ax.text(0.03, 0.475, "LEARNING FROM UNLABELED PANGENOME GRAPHS", fontsize=8.5, color=COLORS["muted"], weight="bold")
    ax.text(0.03, 0.425, "E", fontsize=15, weight="bold", color=COLORS["teal"])
    ax.text(0.068, 0.425, "Learn once, reuse the graph profiles across analyses", fontsize=13, weight="bold")

    box(ax, 0.05, 0.19, 0.18, 0.15, "Graph-trained model", "kept unchanged after learning", face=COLORS["teal_light"], edge=COLORS["teal"])
    arrow(ax, (0.23, 0.265), (0.31, 0.265), COLORS["teal"])
    box(ax, 0.31, 0.19, 0.16, 0.15, "Learned profiles", "segment / region characteristics", face="white", edge=COLORS["purple"])
    destinations = [
        (0.55, 0.31, "Transfer to another graph", COLORS["blue"], COLORS["blue_light"]),
        (0.76, 0.31, "Regulatory-element prediction", COLORS["gold"], COLORS["gold_light"]),
        (0.55, 0.10, "Structural-variant prediction", COLORS["rose"], COLORS["rose_light"]),
        (0.76, 0.10, "Future biological analyses", COLORS["green"], COLORS["green_light"]),
    ]
    ...
    ax.text(
        0.05,
        0.08,
        "During pretraining, the graph model learns from genomic positions and connections rather than reading the DNA sequence itself.",
        fontsize=9,
        color=COLORS["muted"],
    )
